...
#define LIGHT (13)    /*For making the light blink*/
...
{ 
  ...
  while (1)
  {
    ...
    // If there is data stored in incomingData, send it to parse_command;
    if( i > 0 ){
      parse_command();
    }
...
void Dis_7SEG (int Decimal, byte High, byte Low, bool sign)
{
  ...
  if (Digit > 0 && C_or_F == 0)                 /* Display "c" if there is more space on 7-SEG */
  {
    Send7SEG (Digit,0x58);
    Digit--;
  } else if (Digit > 0 && C_or_F == 1) {
    Send7SEG (Digit,0x71);
    Digit--;
  }
...
/* Method to parse incomingData command from sever */
void parse_command() {
  
   // If the control signal is changing from C to F or back
   if( incomingData[0] == 'F' ){
     // Also updates threshold values, if necessary
     if( C_or_F == 0 ){
       hot = ((float) hot) * 1.8 + 32.0;
       cold =  ((float) cold) * 1.8 + 32.0;
     }
     C_or_F = 1;
   }
    else if( incomingData[0] == 'C' ){
      if( C_or_F == 1 ){
        hot = (( (float) hot ) - 32 ) * 0.5556;
        cold = (( (float) cold ) - 32 ) * 0.5556;
      }
    ...
    if( strstr( incomingData, "THRESHOLD" ) ){
      
      strtok( incomingData, " " );

      // Points to cold
      tok = strtok( NULL, " " );
      String coldString(tok);
      cold = coldString.toInt();

      // Points to hot
      tok = strtok( NULL, " ");
      String hotString(tok);
      hot = hotString.toInt();
      digitalWrite( RED, HIGH);  
    }

    if( strstr( incomingData, "disp" ) ){
      strtok( incomingData, "disp" );
      tok = strtok( NULL, "=" );
      if( strcmp( tok, "F" ) == 0 ){
        C_or_F = 1;
      } else{
    ...
    if( strstr( incomingData, "FAN=ON" ) ){
        // fan on
        analogWrite(enA, 255);
        digitalWrite( RED, HIGH);
      }

    if( strstr( incomingData, "FAN=OFF") ){
        // fan off
        analogWrite(enA, 0);
        digitalWrite(RED, LOW);
      }
